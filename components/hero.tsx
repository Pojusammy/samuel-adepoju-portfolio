...
import { heroContent, heroSummary, heroSummaryMobile } from "@/data/site";
...
export function Hero() {
  ...
  const mobileParagraphs = isMobileExpanded ? heroSummary : heroSummaryMobile;
      ...
      >
        ...
        <div className="space-y-5 sm:hidden">
          {mobileParagraphs.map((paragraph, index) => (
            <motion.p
              key={`mobile-${isMobileExpanded ? "full" : "summary"}-${index}`}
              className="max-w-[42.5rem]"
              variants={{
                hidden: { opacity: 0, y: 12 },
                visible: { opacity: 1, y: 0 },
          ...
          {!isMobileExpanded ? (
            ...
            >
              <button
                ...
                className="inline-flex text-[13px] font-medium tracking-[-0.01em] text-foreground transition-colors hover:text-foreground-soft"
              >
                <span
